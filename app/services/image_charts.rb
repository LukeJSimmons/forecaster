class ImageCharts < ApplicationService
  ...
  def call
    ...
    "https://image-charts.com/chart?cht=lc&chd=a:#{max_temps.join(',')}|#{min_temps.join(',')}&chl=#{max_temps.join('|')}|#{min_temps.join('|')}&chlps=align,50&chs=700x350&chxt=x,y&chxl=0:|#{dates.join('|')}&chdl=High|Low&chdlp=t&chco=a51d2a,0250c4"
  end
end